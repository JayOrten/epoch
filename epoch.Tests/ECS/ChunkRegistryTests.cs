...
public class ChunkRegistryTests
{
    ...
    public ChunkRegistryTests()
    {
        _world = World.Create();
        _registry = new ChunkRegistry(_world, 4, 16);
    }
    ...
    [Fact]
    public void IsPassableAt_MissingChunk_ReturnsTrue()
    {
        // No chunk loaded → passable (empty space)
        Assert.True(_registry.IsPassableAt(new Vector3(99, 99, 5)));
    }

    [Fact]
    public void IsPassableAt_EmptyChunk_ReturnsTrue()
    {
        // Chunk exists but no entity → passable (open sky)
        _registry.EnsureChunk(0, 0);
    ...
    [Fact]
    public void CrossChunk_Lookup()
    {
        // chunkSize=4, so (3,0,0) and (4,0,0) are in different chunks
    ...
    [Fact]
    public void RemoveChunk_ClearsAndDestroysEntities()
    {
    ...
    [Fact]
    public void RemoveChunk_NonexistentChunk_DoesNotThrow()
    {
        _registry.RemoveChunk(99, 99);
    }

    [Fact]
    public void RemoveChunk_DestroysAcrossZLevels()
    {
