...
namespace epoch.Engine.Graphics.Tiles.TileBatches
{
    ...
    public class TileBatch : IDisposable
    {
        ...
        public TileBatch(GraphicsDevice graphicsDevice, int capacity)
        {
            if (graphicsDevice == null)
            {
                throw new ArgumentNullException("graphicsDevice", "graphicsDevice cannot be null.");
            }
            ...
            _batcher = new TileBatcher(graphicsDevice, capacity);
        ...
        )
        {
            ...
            switch (_sortMode)
            {
                ...
                case SpriteSortMode.Texture:
                    // Changed from SortingKey to GetHashCode because SortingKey is internal.
                    item.SortKey = texture.GetHashCode();
                    break;
                // Comparison of Depth
            ...
            if (sourceRectangle.HasValue)
            {
                ...
                // Changed from TexelWidth/TexelHeight to Width/Height because TexelWidth is internal.
                float texelWidth = 1.0f / texture.Width;
                float texelHeight = 1.0f / texture.Height;
                _texCoordTL.X = srcRect.X * texelWidth;
                _texCoordTL.Y = srcRect.Y * texelHeight;
                _texCoordBR.X = (srcRect.X + srcRect.Width) * texelWidth;
                _texCoordBR.Y = (srcRect.Y + srcRect.Height) * texelHeight;
            }
            else
            {
        ...
        )
        {
            ...
            if (sourceRectangle.HasValue)
            {
                ...
                if (srcRect.Width != 0)
                    ...
                    origin.X = origin.X * (float)destinationRectangle.Width * texelWidth;
                if (srcRect.Height != 0)
                    origin.Y =
                        origin.Y * (float)destinationRectangle.Height / (float)srcRect.Height;
                else
                    origin.Y = origin.Y * (float)destinationRectangle.Height * texelHeight;
            }
            else
            {
                _texCoordTL = Vector2.Zero;
        ...
        )
        {
            ...
            // set SortKey based on SpriteSortMode.
            // Replaced SortingKey with GetHashCode because SortingKey is internal.
            item.SortKey = _sortMode == SpriteSortMode.Texture ? texture.GetHashCode() : 0;

            Vector2 size;

            // Changed from TexelWidth/TexelHeight to Width/Height because TexelWidth is internal.
            // Calcualtes the normalized Texel size: size of one pixel in the texture in uv space.
            float texelWidth = 1.0f / texture.Width;
            float texelHeight = 1.0f / texture.Height;
