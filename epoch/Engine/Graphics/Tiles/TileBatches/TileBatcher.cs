...
namespace epoch.Engine.Graphics.Tiles.TileBatches
{
    ...
    internal class TileBatcher
    {
        ...
        /// </summary>
        private TileBatchItem[] _batchItemList;
        ...
        private TileVertex[] _vertexArray;

        public TileBatcher(GraphicsDevice device, int capacity = 0)
        {
            _device = device;
            ...
            _batchItemList = new TileBatchItem[capacity];
            _batchItemCount = 0;

            for (int i = 0; i < capacity; i++)
                _batchItemList[i] = new TileBatchItem();
        ...
        /// </summary>
        /// <returns></returns>
        public TileBatchItem CreateBatchItem()
        {
        ...
        private unsafe void EnsureArrayCapacity(int numBatchItems)
        {
            ...
            _vertexArray = new TileVertex[4 * numBatchItems];
        }
        ...
        public unsafe void DrawBatch(SpriteSortMode sortMode, Effect effect)
        {
            ...
            // Removed because it's just for debugging?
            // unchecked
            // {
            ...
            while (batchCount > 0)
            {
                ...
                fixed (TileVertex* vertexArrayFixedPtr = _vertexArray)
                {
                    ...
                    for (
                        int i = 0;
                        i < numBatchesToProcess;
                        i++, batchIndex++, index += 4, vertexArrayPtr += 4
                    )
                    {
                        TileBatchItem item = _batchItemList[batchIndex];
                        ...
                        if (shouldFlush)
                        {
                            ...
                            _device.Textures[0] = tex; // Binds the texture here
                        }
